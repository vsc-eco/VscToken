...
contract('ERC20', function ([_, owner, recipient, anotherAccount]) {
  beforeEach(async function () {
    ...
    this.token.transfer(owner, TWO_BILLION_IN_WEI)
  });

  describe('total supply', function () {
    it('returns the total amount of tokens', async function () {
      (await this.token.totalSupply()).should.be.bignumber.equal(TWO_BILLION_IN_WEI);
    });
  });
  ...
  describe('balanceOf', function () {
    ...
    describe('when the requested account has some tokens', function () {
      it('returns the total amount of tokens', async function () {
        (await this.token.balanceOf(owner)).should.be.bignumber.equal(TWO_BILLION_IN_WEI);
      });
    });
  });
  ...
  describe('transfer', function () {
    describe('when the recipient is not the zero address', function () {
      ...
      describe('when the sender does not have enough balance', function () {
        const amount = TWO_BILLION_AND_ONE_WEI;
      ...
      describe('when the sender has enough balance', function () {
        const amount = TWO_BILLION_IN_WEI;
  ...
  describe('transfer from', function () {
    ...
    describe('when the recipient is not the zero address', function () {
      ...
      describe('when the spender has enough approved balance', function () {
        beforeEach(async function () {
          await this.token.approve(spender, TWO_BILLION_IN_WEI, { from: owner });
        });
